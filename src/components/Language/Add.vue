<template>
  <q-card :style="{
    ...
    <q-card-section class="q-pt-none">
      <q-input :autofocus="true" v-model="language.code" :label="$t(Utils.getKey('Locale Web'))" dense outlined
        maxlength="20" lazy-rules :rules="[
...
async function onSubmit() {
  try {
    ...
    if (!Utils.containsOnlyCharacterAllLanguage(language.value.name) || !Utils.containsOnlyCharacterAllLanguage(language.value.code) || !Utils.containsOnlyCharacterExeptDash(language.value.locale)) {
      // $q.notify({
      //   position: "top-right",
      //   type: "negative",
      //   icon: "fas fa-check",
