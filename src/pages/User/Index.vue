<template>
  <q-page>
    <q-card style="margin-top:20px; margin-left:0px; box-shadow: none; min-height:85vh;">
      <q-card-section>
        <q-table flat class="q-pt-md" color="primary" :loading="loading" :rows="items" row-key="id" :columns="columns"
          ...
          :rows-per-page-options="[10, 15, 20, 50, 100, 150, 200, 500]" binary-state-sort v-model:selected="selected"
          :rows-per-page-label="$t(Utils.getKey('Records per page'))">
          <template v-slot:top>
            <q-input dense outlined debounce="300" v-model="filters.name" :placeholder="$t(Utils.getKey('Search'))"
              style="width: 300px" />
            ...
            <q-space />
            <add-button v-if="Utils.hasPermissions(['User: Create User'])" :disable="loading" @click="showAdd = true"
              tooltipText="Add" color="primary" />
          </template>
          ...
          <template v-slot:body-cell-enable_ga="props">
            <q-td class="text-center">
              <div>
                <q-toggle ref="toggleRef" :modelValue="props.row.enableGa" checked-icon="check" color="green" :label="props.row.enableGa == 0
                  ? $t(Utils.getKey('off'))
                  : $t(Utils.getKey('on'))
                  " unchecked-icon="clear" :true-value="1" :false-value="0" size="50px"
          ...
          <template v-slot:body-cell-actions="props">
            <q-td class="text-center">
              <q-btn v-if="Utils.hasPermissions(['User: Edit/Update User']) &&
                props.row.id !== 1
                " class="q-ml-sm" size="xs" rounded padding="5px" color="primary" icon="fas fa-pen"
                @click="onEditClick(props.row)">
                <q-tooltip>{{ $t(Utils.getKey("Edit")) }}</q-tooltip>
              </q-btn>

              <q-btn v-if="Utils.hasPermissions(['User: Edit/Update User'])" class="q-ml-sm" size="xs" rounded
                padding="5px" color="primary" icon="mdi-filter-remove-outline" @click="onResetClick(props.row)">
              ...
              <q-btn v-if="Utils.hasPermissions(['User: Edit/Update User']) &&
                ...
                " class="q-ml-sm" size="xs" rounded padding="5px" color="primary" icon="mdi-google-plus"
              ...
              <q-btn v-if="!props.row.roles?.map((role) => role.name)
                    .includes('super admin') &&
                  Utils.hasPermissions(['User: Delete User']) &&
                  props.row.email != 'admin@mail.com'
                  " class="q-ml-sm" size="xs" rounded padding="5px" color="negative" icon="fas fa-trash"
    ...
    <q-dialog v-model="showEdit" position="top" persistent>
      <edit-user :data="selectedUser" @onClose="showEdit = false" @onUpdated="onRefresh" />
    </q-dialog>

    <q-dialog v-model="resetPassword" persistent>
      <reset :data="selectedUser" @onClose="resetPassword = false" @onUpdated="resetPassword = false" />
    </q-dialog>

    <q-dialog v-model="showConfirm" persistent>
      <Confirm :message="`Are you sure you want to delete this User`" @cancel="showConfirm = false"
        @confirm="onDelete('User')" :deleting="deleting" />
    </q-dialog>
    <q-dialog v-model="showGoogleKeyConfirm" persistent>
      <Confirm :message="`Reset google authenticator`" @cancel="showGoogleKeyConfirm = false" @confirm="updateUser()"
        :deleting="saving" :button-label="$t(Utils.getKey('confirm'))" />
    </q-dialog>

    <q-dialog v-model="showDisbleGauth" persistent>
      <Confirm :message="`Turn ${selectedUser.enable_ga == true ? 'Off' : 'On'
        } google authenticator`" @cancel="showDisbleGauth = false" @confirm="updateUser()"
